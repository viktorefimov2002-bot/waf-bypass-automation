...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="waf-bypass-tool", description="Normalize, verify and analyze nemesida/waf-bypass reports")
    ...
    command.add_argument("--overrides", type=_path)
    command.add_argument("--output", required=True, type=_path)

    command = subparsers.add_parser("report", help="Create an XLSX report from normalized/rechecked JSONL")
    command.add_argument("--input", required=True, type=_path)
    command.add_argument("--groups", required=True, type=_path)
    command.add_argument("--taxonomy", type=_path)
    command.add_argument("--output", required=True, type=_path)

    command = subparsers.add_parser("recheck", help="Dry-run or execute cURL variants")
    command.add_argument("--input", required=True, type=_path)
    command.add_argument("--output", required=True, type=_path)
    ...
    command = subparsers.add_parser("diff", help="Compare two normalized/rechecked runs")
